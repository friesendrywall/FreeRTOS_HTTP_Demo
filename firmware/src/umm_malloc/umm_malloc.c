...
#include "umm_malloc/umm_malloc_cfg.h"   /* user-dependent */
//#include "uart.h"
...
#define DBGLOG_DEBUG(x,...) //PrintUart(x,##__VA_ARGS__)
#define DBGLOG_TRACE(x,...) //PrintUart(x,##__VA_ARGS__)
/* ------------------------------------------------------------------------- */
...
#define UMM_NUMBLOCKS (heap->umm_numblocks)

/* ------------------------------------------------------------------------ */

#define UMM_BLOCK(b)  (heap->umm_heap[b])
 ...
 */
static void umm_split_block(_Heap * heap, unsigned long c,
...
static void umm_disconnect_from_free_list(_Heap * heap, unsigned long c) {
...
static void umm_assimilate_up(_Heap * heap, unsigned long c) {
...
static unsigned long umm_assimilate_down(_Heap * heap, unsigned long c, unsigned long freemask) {
...
void umm_init(_Heap * heap) {
    ...
    UMM_CRITICAL_ENTRY();
    if (heap->HeapReady) {
	//Another thread was here ahead of us
	UMM_CRITICAL_EXIT();
	return;
    }
#endif
    configASSERT(heap->HeapReady != 0xFF);
    heap->umm_numblocks = (heap->heap_size / sizeof (umm_block));
    memset(heap->umm_heap, 0x00, heap->heap_size);
...
void umm_free(_Heap * heap, void *ptr) {
	...
	c = (((char *)ptr) - (char *)(&(heap->umm_heap[0]))) / sizeof(umm_block);
...
#error "No UMM_*_FIT is defined - check umm_malloc_cfg.h"
	...
	if (blockSize == blocks) {
	     ...
	     */
	    umm_split_block(heap, cf, blocks,
...
void *umm_realloc(_Heap * heap, void *ptr, size_t size) {
	...
	umm_free(heap, (void *) &UMM_DATA(c + blocks));
    } else {
...
void *umm_calloc(_Heap * heap, size_t num, size_t item_size ) {
  void *ret;
